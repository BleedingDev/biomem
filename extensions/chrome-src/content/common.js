(function () {
  ...
  function log(msg) {
    ...
    // console.log(`[biomem Hippocampus plugin] ${msg}`);
  }
  ...
  function updatePanel(connected) {
    ...
    if (!connected) {
      text.textContent = "biomem: software not running";
  ...
  function startAssistantObserver(adapter) {
    ...
    const scheduleFinalize = (pendingStore, assistantSnapshot, text) => {
      ...
      if (text !== pendingStore.lastAssistantText) {
        ...
        if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] observer: scheduling finalizeAssistant in 1500ms, text length:", text.length, "hasPAM:", /\|STPAM\|/.test(text));
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      if (shouldIgnoreAssistantCandidate(text)) {
        if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] observer: ignored candidate, text length:", text.length);
  ...
  async function finalizeAssistant(adapter, lastAssistant, expectedPendingStore) {
    if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] finalizeAssistant ENTERED, pendingStore:", !!STATE.pendingStore);
    const pendingStore = expectedPendingStore || STATE.pendingStore;
    if (!pendingStore || STATE.pendingStore !== pendingStore || pendingStore.inFlight) { if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] EXIT: no available pendingStore"); return; }
    ...
    if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] single-element rawText length:", (rawText || "").length, "hasPAM:", /\|STPAM\|/.test(rawText || ""));
      ...
      if (adapter.getAssistantMessageElements) {
        ...
        if (allAssistant.length > 0) {
          ...
          if (combined && /\|STPAM\|[\s\S]*?\|ENDPAM\|/i.test(combined)) {
            if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] found PAM tokens via multi-element scan, combined length:", combined.length);
            rawText = combined;
          }
        }
      }
    ...
    if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] final rawText length:", (rawText || "").length, "hasPAM:", /\|STPAM\|/.test(rawText || ""));
    if (!rawText) { if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] EXIT: no rawText"); return; }
    ...
    if (STATE.lastTurn && normalizeText(rawText) === normalizeText(STATE.lastTurn.userText || "")) {
      if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] EXIT: text matches user input (wrong element captured), NOT clearing pendingStore");
      return;
    }

    if (shouldIgnoreAssistantCandidate(rawText)) {
      if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] EXIT: shouldIgnoreAssistantCandidate returned true");
    ...
    if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] parsed.hasTokens:", parsed.hasTokens, "hasStoreText:", !!storeModelText);
    if (storeModelText) {
      ...
      if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] CALLING store(), hasSession:", !!pendingStore.sessionId);
      try {
        ...
        if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] store() SUCCEEDED:", !!storeResult);
      } catch (err) {
        if (DEBUG_PROMPT_IO) console.error("[biomem STORE DEBUG] store() FAILED, code:", err && err.code ? err.code : "unknown");
        log(`store failed: ${err.message}`);
      }
    } else {
      if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] SKIPPED store(): no stable assistant text");
    ...
    if (parsed.hasTokens || parsed.hadArtifacts) {
      ...
      function doPamCleanup(passLabel) {
        try {
          ...
          if (cleaned) {
            if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] PAM tokens cleaned (" + passLabel + ")");
          }
        } catch (cleanErr) {
          if (DEBUG_PROMPT_IO) console.log("[biomem STORE DEBUG] PAM cleanup error (" + passLabel + ", non-fatal):", cleanErr.message);
        }
      }
